	...
	\begin{columns}
		\begin{column}{0.4\textwidth}
			...
			\begin{itemize}
				...
				\item \uncover<3->{{\color{blue} (((No Prey Character)))}}
			\end{itemize}
		\end{column}
		\begin{column}{0.60\textwidth}
			{\bf Parameters}
...
\begin{frame}
	...
	\uncover<2->{{\bf Attack Rate is a Function of the {\color{blue}Prey's Trait Value} and the {\color{red}Predator's Trait Value}}
	\begin{align*}
	...
	\begin{columns}
		...
		\begin{column}{0.48\textwidth}
			...
			\begin{itemize}
				...
				\item $\sigma^2 \equiv $ Predator Trait Variance
				\item \uncover<2->{$\alpha \equiv $ Maximum attack rate}
				\item \uncover<2->{{\color{cyan}$\theta \equiv $ Optimal trait \bf difference}}
				\item \uncover<2->{$\tau \equiv $ Specialization Constant}
			\end{itemize}
		\end{column}
	\end{columns}
...
\begin{frame}
	...
	\begin{center}
		\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3.5cm, thick,main node/.style={circle,fill=blue!20,draw,font=\sffamily\Large\bfseries}]
			...
			\path[every node/.style={font=\sffamily\small}]
			(input1)	edge node {\tiny{\color{blue}$r_jN_j\left(1 - \frac{N_j}{K_j}\right)$}} (n1)
			(n1)		edge node {\tiny{\color{blue}$\overline{a_{i,j}}(\overline{n_j}, \overline{m_i})M_iN_j$}} (midpt1)
					edge node {\tiny{\color{blue}$\overline{a_{i-1,j}}(\overline{n_j}, \overline{m_{i-1}})M_{i-1}N_j$}} (furtherright)
					edge node {\tiny{\color{blue}$\overline{a_{i+1,j}}(\overline{n_j}, \overline{m_{i+1}})M_{i+1}N_j$}} (alittleleft)
			(midpt1)	edge node {\tiny{\color{red}$e_{i,j}\overline{a_{i,j}}(\overline{n_j}, \overline{m_i})M_iN_j$}} (m1)
			(m1)		edge node {\tiny{\color{red}$d_iM_i$}} (output1)
			(upleft)	edge node {\tiny{\color{red}$e_{i,j-1}\overline{a_{i,j-1}}(\overline{n_{j-1}}, \overline{m_i})M_iN_{j-1}$}} (m1)
			(alittleright)	edge node {\tiny{\color{red}$e_{i,j-1}\overline{a_{i,j-1}}(\overline{n_{j-1}}, \overline{m_i})M_iN_{j-1}$}} (m1)
			;
		\end{tikzpicture}\vskip0.35cm
	\end{center}
	}
