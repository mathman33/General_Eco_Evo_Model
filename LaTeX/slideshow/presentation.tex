...
\begin{frame}
	...
	\begin{align*}
		(N^*, M^*, \overline{n}^*, \overline{m}^*) = (K, 0, \mu^*, \mu^* + \theta)\ \ \ \ \ \text{where $\mu^*$ is arbitrary}
	\end{align*}
...
\begin{frame}
	...
	\begin{itemize}
		\item \uncover<3->{$\dfrac{\sigma_G^2}{\beta_G^2} > \dfrac{r}{d}\left(1 - \dfrac{d\sqrt{A}}{Ke\alpha\tau}\right)$}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{align*}
		(N^*, M^*, \overline{n}^*, \overline{m}^*) = (\dfrac{d\sqrt{A}}{e \alpha \tau}\ ,\ \dfrac{\rho\gamma\sqrt{A}}{\alpha\tau\sqrt{B}}\left(1 - \dfrac{d\sqrt{A}}{Ke\alpha\tau}\right)\ ,\ \phi\ ,\ \theta + \phi) \\[.1cm]
