...
\section{Summary}
The novelty of our model resides in the incorporation of coevolutionary dynamics with realistic and natural Gaussian functions and density-dependent prey growth rates.  The attack rate, in contrast to that of Schreiber et. al., is dependent on both predator and prey trait values.  In addition, prey growth rate is a Gaussian function dependent of the prey's trait value.

Outline of the paper.........

In the first model, possible dynamics include the extinction of both predator and prey, exclusion of the predator, a coexistence state, and an ``arms race'' dynamic.  We note that in order to for the exclusion state to be stable, the predator death rate must be high enough (see (\ref{exclusion_stability_MODEL1})).  For coexistence, we see that the predator must be ``fast'' enough at evolving in order for it to have any chance of survival.  The ``arms race'' coexistence dynamic is not found in classical ecological models, and can possibly be interpreted as speciation.

The second model's dynamics include the extinction of both predator and prey, exclusion of predator, and a coexistence state.  The conditions for stability of both exclusion and coexistence are parallel to that of the first model.  In contrast to the first model, the non-stable coexistence dynamic shows cyclic behavior.  The introduction of stabilizing selection for the prey prevented the evolutionary ``arms'' race.  Instead, the second model provides long-term Red Queen Dynamics \cite{Khibnik_Kondrashov_1997} and possibly unique, stable limit cycles.
...
The implications of our work are......

Our future work includes proving Hopf Bifurcation for the second model, the expansion of analytical results for more complex predator and prey interactions (intraguild, apparent competition, direct competition), and the incorporation of direct competition between prey for resources.
